...
test('test_getting_started_page_excel_download', async ({page}) => {
    ...
    if (popupOrDownload.url) {
        ...
        console.log('Excel download URL:', download.url());
        expect(download.url()).toBe(
            "https://www.cpf.gov.sg/content/dam/web/member/faq/general-information---useful-tips/documents/Guide_to_view_and_save_CPF_statements.xlsx"
        );
    } else {
        // It's a popup
        const popup = popupOrDownload;
...
test('test_upload_technical_results_page', async ({page}) => {
    test.setTimeout(1200000);
    let workspace_name = 'workspace_1' + Math.floor(Math.random() * 1000000000);
    console.log(workspace_name)
    await page.goto('http://127.0.0.1:8501');
    ...
    const test_result_json_path = path.resolve(__dirname, '..', 'test-data', 'my-run-attack-5-5.json');
    ...
    await expect(page.locator(`text=my-run-attack-5-5.json`)).toBeVisible();
    await expect(page.getByText('File uploaded successfully')).toBeVisible();
    await expect(page.getByTestId('stFileUploaderFileName')).toContainText('my-run-attack-5-5.json');
...
test('test_upload_generate_report_page_start_over_btn', async ({page}) => {
    test.setTimeout(1200000);
    let workspace_name = 'workspace_1' + Math.floor(Math.random() * 1000000000);
    console.log(workspace_name)
    await page.goto('http://127.0.0.1:8501');
...
test('test_upload_technical_results_page_start_over_btn', async ({page}) => {
    test.setTimeout(1200000);
    let workspace_name = 'workspace_1' + Math.floor(Math.random() * 1000000000);
    console.log(workspace_name)
    await page.goto('http://127.0.0.1:8501');
