...
test('test_getting_started_page', async ({page}) => {
    ...
    // Click the link (adjust role/name as needed)
    await page.getByRole('link', {name: 'Download Testing Framework - Excel'}).click();

    // Wait for popup to be created
    const page2 = await page2Promise;

    // ✅ Ensure the popup fully loads/navigates
    await page2.waitForLoadState('load');  // or 'domcontentloaded'
