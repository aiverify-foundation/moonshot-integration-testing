...
def test_cli_delete_dataset():
    ...
    # Copy file
    copy_file(CLI_DIR + '/moonshot-data/datasets/bbq-lite-age-ambiguous.json')

    command = 'delete_dataset bbq-lite-age-ambiguous ' + '\n'
    print('Command:', command)
    # Example command to send to the process
    process.stdin.write(command)
    process.stdin.flush()
    ...
    print('=========================Output Last Line:', last_line)
    # Restore file
    current_path = CLI_DIR + "/moonshot-data/datasets/copy_of_bbq-lite-age-ambiguous.json"  # Replace with the current file path
    new_name = "bbq-lite-age-ambiguous.json"  # Specify the new file name
    rename_file(current_path, new_name)
    assert last_line == "Are you sure you want to delete the dataset (y/N)? [delete_dataset]: Dataset deleted."
